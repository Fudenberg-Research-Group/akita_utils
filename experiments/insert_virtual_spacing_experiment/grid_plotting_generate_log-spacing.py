...
def main():
    ...
    parser.add_option(
        ...
        default=2,
        type="int",
        help="Specify CTCF motif sequence index to be tested",
    )
    parser.add_option(
        "--orientation-string",
    ...
    spacing_list = [int(i) for i in options.spacer_list.split(",")]
    ...
    (expected_df_len, observed_df_len) = validate_df_lenght(num_orients, len(background_indices_list), len(spacing_list), df_with_flanks_spacers)
...
def validate_df_lenght(num_orientations, num_backgrounds, num_spacings, df):
