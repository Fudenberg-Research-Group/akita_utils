...
from akita_utils import split_df_equally, symmertic_insertion_seqs_gen
...
def initialize_output_h5(
    ...
    for target_ind in range(num_targets):
        scd_out.create_dataset(
            f"pred_h{head_index}_m{model_index}_t{target_ind}",
            shape=(num_experiments, 130305),
            dtype="float16",
            compression=None,
        )
...
def write_snp(
    ...
    for target_ind in range(ref_preds.shape[1]):
        scd_out[f"pred_h{head_index}_m{model_index}_t{target_ind}"][
            si
        ] = ref_preds[:, target_ind].astype("float16")
