...
Compute SNP Contact Difference (SCD) scores for motifs in tsv motif file.
...
# define sequence generator
  def mask_central_seq(seq_1hot, motif_width=20):
    ...
    mask_inds = np.arange(seq_length//2-motif_width//2, 
                          seq_length//2+motif_width//2)
    seq_1hot_perm[mask_inds,:]= 0
    return seq_1hot_perm
  
  def permute_central_seq(seq_1hot, motif_width=20):
    seq_1hot_perm = seq_1hot.copy()   
    central_inds = np.arange(seq_length//2-motif_width//2, 
                          seq_length//2+motif_width//2)
  ...
  def fetch_centered_padded_seq(chrom, start, end):
      mid = (start+end)//2
      start_centered, end_centered = int(mid- seq_length//2), int(mid+seq_length//2)
    ...
    for s in seq_coords_df.itertuples():
      ...
      if use_span:
        ...
        if mutation_method == 'mask':
          ...
          list_1hot.append( permute_central_seq(wt_1hot, motif_width=motif_width))
  ...
  scd_out = initialize_output_h5(options.out_dir, options.scd_stats,
                                 seq_coords_df, target_ids, target_labels)
  ...
  for si in range(num_motifs):
    ...
    # process SNP
    write_snp(ref_preds, alt_preds, scd_out, si, seqnn_model.diagonal_offset, options.scd_stats,
              plot_dir, options.plot_lim_min, options.plot_freq)
...
def initialize_output_h5(out_dir, scd_stats, seq_coords_df, target_ids, target_labels):
  ...
  for scd_stat in scd_stats:
    if 'INS' not in scd_stat:
      ...
      scd_out.create_dataset('ref_'+scd_stat,
        shape=(num_motifs, num_targets), dtype='float16', compression=None)     
      scd_out.create_dataset('alt_'+scd_stat,
        shape=(num_motifs, num_targets), dtype='float16', compression=None)     
...
def write_snp(ref_preds, alt_preds, scd_out, si, diagonal_offset, scd_stats=['SSD','SCD'],
  ...
  if np.any((["INS" in i for i in scd_stats])):
    ...
    for stat in scd_stats:
      if 'INS' in stat:
        ...
        scd_out['ref_'+stat][si,:] = insul_diamonds_scores(ref_map, window=insul_window)
        scd_out['alt_'+stat][si,:] = insul_diamonds_scores(alt_map, window=insul_window) 
