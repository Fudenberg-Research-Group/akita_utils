...
def main():
    ...
    parser.add_option(
        ...
        default=2,
        type="int",
        help="Specify number of strong CTCFs",
    )
    parser.add_option(
        "--num-weak",
        ...
        default=2,
        type="int",
        help="Specify number of weak CTCFs",
    )
    parser.add_option(
        "--mode",
        dest="mode",
        default="same",
        type="str",
        help="Specify mode of adding orientation strings list: same, customize, weak_strong, for_each",
    )
    parser.add_option(
        "--orientation-list",
        dest="orientation_list",
        default=">>,>>,>>,>>",
        type="string",
        help="Specify orientation strings, they have to be written within the quotation marks and comma-separated",
    )
    ...
    orient_list = options.orientation_list.split(",")
    
    if options.mode=="same" and len(orient_list)!=1:
        parser.error("When mode same chosen, only one orientation has to be provided")
    
    if options.mode=="customize" and len(orient_list)!=(options.num_strong_motifs + options.num_weak_motifs):
        parser.error("When mode customize chosen, length of the list of orientation strings provided had to be equal to the sum of number of strong and weak CTCF motifs")
    
    if options.mode=="weak_strong" and len(orient_list)!=2:
        parser.error("When mode weak_strong chosen, you should provide a list of two orientation strings: first of strong motifs, second - for weak")
