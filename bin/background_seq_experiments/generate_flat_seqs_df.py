...
################################################################################
def main():
    #--------------------------------------on boarding (these will be transformed to args)------------------------------------------------
    ...
    new_dataframe = filter_sites_by_score(dataframe,score_key="GC",upper_threshold=100,lower_threshold=0,mode="flat",num_sites=90) # carefull will mode
    ...
    parameters_combo_dataframe.to_csv(f'{storage_folder}/flat_seqs.tsv', sep='\t', index=False)
