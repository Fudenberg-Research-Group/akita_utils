...
def main():
    ...
    parser.add_argument('-sample_size', dest='sample_size', type=int, help='specific number of seqs to sample')
    parser.add_argument('-specific_locus', nargs='+', type=int, dest='specific_locus', help='list of specific seqs indicies in dataframe')
    
    args = parser.parse_args()
    
    seqs_df = pd.read_csv(args.shffled_seqs_tsv, sep="\t")
    create_shuffled_seqs(args.genome_fasta, dataframe=seqs_df, output_dir=args.output_dir,sample_size=args.sample_size,specific_locus=args.specific_locus)
