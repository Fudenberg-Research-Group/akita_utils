...
def _multi_insert_casette(
    seq_1hot, seq_1hot_insertions, spacer_bp, orientation_string
):

    assert len(seq_1hot_insertions) == len(
        orientation_string
    ), "insertions dont match orientations, please check"
    seq_length = seq_1hot.shape[0]
    total_insert_bp = sum(
        [len(insertion) for insertion in seq_1hot_insertions]
    )
    ...
    for i in range(num_inserts):
        ...
        offset = (
            insert_start_bp + length_of_previous_insert + spacer_bp
        )  # i * inserts_plus_spacer_bp + spacer_bp
        length_of_previous_insert += (
            len(seq_1hot_insertions[i]) + 2 * spacer_bp
        )
        if orientation_arrow == ">":
            output_seq[offset : offset + insert_bp] = seq_1hot_insertions[i]
        else:
            output_seq[
                offset : offset + insert_bp
            ] = akita_utils.dna_utils.hot1_rc(seq_1hot_insertions[i])
    return output_seq
...
def fixed_core_insertion_seqs_gen(seq_coords_df, background_seqs, genome_open):
    ...
    for s in seq_coords_df.itertuples():
        ...
        seq_1hot_flank = dna_1hot(
            genome_open.fetch(
                flank_chrom, flank_start - flank_bp, flank_end + flank_bp
            ).upper()
        )
        if flank_strand == "-":
        ...
        seq_1hot_core = dna_1hot(
            genome_open.fetch(core_chrom, core_start, core_end).upper()
        )
        if core_strand == "-":
        ...
        seq_1hot_insertion = np.concatenate(
            (seq_1hot_flank_left, seq_1hot_core, seq_1hot_flank_right), axis=0
        )
...
def modular_insertion_seqs_gen(seq_coords_df, background_seqs, genome_open):
    ...
    for s in seq_coords_df.itertuples():
        ...
        seq_1hot = _multi_insert_casette(
            seq_1hot, seq_1hot_insertions, spacer_bp, orientation_string
        )
...
def randomise_spans(seq_1hot, spans, motif_window):
    ...
    for s in spans:
        start, end = s - half_motif_window, s + half_motif_window
        seq_1hot_perm[start:end] = random_seq_permutation(
            seq_1hot_perm[start:end]
        )
    return seq_1hot_perm
