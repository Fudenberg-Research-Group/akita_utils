...
def create_flat_seqs_gen(
    seqnn_model, genome_fasta, dataframe, max_iters=10, batch_size=6
):
    ...
    for ind in range(num_seqs):
        ...
        while num_iters < max_iters:
            ...
            (
                ...
                scores_pixelwise,
                custom_scores,
            ) = _check_and_append_satisfying_seqs(
...
def _calculate_scores_from_predictions(predictions):
    ...
    for seq_num in range(predictions.shape[0]):
        ...
        scores_pixelwise += [
            np.max(np.max(np.abs(ref_preds), axis=0))
        ]  # maximum_of_MPS
        scores += [np.max(np.sqrt((ref_preds**2).sum(axis=0)))]  # maximum_of_SCD
        custom_score += [
            np.min(3 / mean + 2 / std)
        ]  # minimum_of_CS  keep in mind custom_score = np.min(3 / (mean * np.log(mean)) + 2 / std_dev)
...
def _check_and_append_satisfying_seqs(
    ...
    """
    scores, scores_pixelwise, custom_scores = _calculate_scores_from_predictions(
        predictions
    )

    for score_ind, score in enumerate(scores):
        if (
            scores[score_ind] < scores_thresh
            and scores_pixelwise[score_ind] < scores_pixelwise_thresh
            and custom_scores[score_ind] > custom_scores_thresh
        ):
            num_iters = max_iters
            best_ind = score_ind
            best_seq = seq_1hot_batch[best_ind]
            best_pred = predictions[best_ind]
            best_score, best_score_pixelwise, best_custom_score = (
                scores[best_ind],
                scores_pixelwise[best_ind],
                custom_scores[best_ind],
            )
            log.info(
                f"success: best seq, map score(SCD) {best_score}, pixelwise(MPS) {best_score_pixelwise}, custom(CS) {best_custom_score}"
            )
            flat_seqs_storage_list.append(
                [
                    best_seq,
                    best_pred,
                    best_score,
