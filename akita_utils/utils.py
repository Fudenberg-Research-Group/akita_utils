...
def create_flat_seqs(
    ...
    for ind in range(num_seqs):
        print(ind)
        chrom, start, end, strand = site_df.iloc[ind][
    ...
    for ind in sample_set:  # range(10,num_seqs):
        ...
        while num_iters < max_iters:
            ...
            if num_iters % devisor == 0:
                flat_seqs[sample_set.index(ind), num_iters] = [
                    best_seq,
                    best_pred,
                    best_score,
            ...
            if num_iters >= max_iters:
                print("max iters exceeded")
                flat_seqs[sample_set.index(ind), "final"] = [
                    best_seq,
                    best_pred,
                    best_score,
                    best_score_pixelwise,
            ...
            for i in range(batch_size):
                ...
                if masking == True:
                    ...
                    for i in np.where(s > ctcf_thresh)[0]:
                        ...
                        seq_1hot_mut[
                            i - motif_window + 1 : i + motif_window
                        ] = seq_1hot_mut[i - motif_window + 1 : i + motif_window][
                            mot_shuf
                        ]
                    seq_1hot_batch.append(seq_1hot_mut)
                else:
