...
from akita_utils.stats_utils import insul_diamonds_scores, calculate_scores
import logging
...
def collect_h5(file_name, out_dir, num_procs):
    ...
    job0_h5_open, final_strings = _initialize_final_h5_file(out_dir, file_name, num_variants)
    
    _write_jobs_h5_to_final_h5(out_dir, file_name, final_h5_open, final_strings, num_procs)
...
def _write_jobs_h5_to_final_h5(out_dir, file_name, final_h5_open, final_strings, num_procs):
    # set values
    vi = 0
    for pi in range(num_procs):
        # open job
